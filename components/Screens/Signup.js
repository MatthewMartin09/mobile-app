...
export const Signup = ({ navigation }) => {
  ...
  const validateContact = () => {
    if (contactType === 'email') {
      return contactInput.toLowerCase().endsWith('@email.com');
    } else {
      // Phone number must be 11 digits
      return /^[0-9]{11}$/.test(contactInput);
    }
  };
  ...
  const handleSignup = async () => {
    if (!name.trim() || !contactInput.trim() || !password.trim() || !dob.trim() || !gender || !barangay) {
    ...
    if (passwordStrength.score < 3) {
      ToastAndroid.show("Password is too weak. Try adding more characters or making it more complex.", ToastAndroid.SHORT);
      return;
    }
    ...
    try {
      ...
      if (users.some(user => (contactType === 'email' ? user.email : user.phone) === trimmedContact)) {
        ToastAndroid.show(`This ${contactType} is already taken`, ToastAndroid.SHORT);
        return;
      }
      ...
      // Add either email or phone based on the type selected
      if (contactType === 'email') {
        newUser.email = trimmedContact;
      } else {
        newUser.phone = trimmedContact;
      }
    
      users.push(newUser);
      await AsyncStorage.setItem('users', JSON.stringify(users));
      await AsyncStorage.setItem('currentUser', JSON.stringify(newUser)); // Store session
      await AsyncStorage.setItem('userName', name.trim()); // Store username
    
      ToastAndroid.show("Signup successful!", ToastAndroid.SHORT);
      navigation.navigate("Login"); // Navigate to login after signup
    } catch (error) {
      ToastAndroid.show("Error saving user", ToastAndroid.SHORT);
